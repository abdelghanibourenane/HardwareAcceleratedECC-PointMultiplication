...
typedef struct {
    ...
    XRadix2wecc_uio_map maps[ MAX_UIO_MAPS ];
} XRadix2wecc_uio_info;

/***************** Variable Definitions **************************************/
static XRadix2wecc_uio_info uio_info;
...
static int uio_info_read_name(XRadix2wecc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRadix2wecc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRadix2wecc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRadix2wecc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRadix2wecc_Initialize(XRadix2wecc *InstancePtr, const char* InstanceName) {
	XRadix2wecc_uio_info *InfoPtr = &uio_info;
